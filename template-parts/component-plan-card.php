...
<div class="!font-inter grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
    ...
    <?php foreach ($plans_data as $plan):
        ...
        <div class="relative flex flex-col gap-4 <?= $card_bg; ?> rounded-[24px] p-8 <?= $card_border; ?> <?= $card_extra; ?> hover:shadow-md transition-all duration-200 overflow-hidden">
            ...
            <?php if ($is_recommended): ?>
                ...
                <div class="absolute top-0 left-0 right-0 overflow-hidden rounded-t-[22px]">
                    ...
                    <div class="relative flex items-center justify-center gap-2 bg-white py-2 px-4">
                        ...
                        <div class="absolute inset-0 -translate-x-full animate-[shimmer_2.2s_ease-in-out_infinite] bg-gradient-to-r from-transparent via-[var(--primary-color)]/10 to-transparent"></div>
            ...
            <!-- Header -->
            <div class="flex items-start gap-3 <?= ($is_best_seller || $is_current || $is_highlighted) && !$is_recommended ? 'mt-4' : ''; ?>">
                <div>
                    <p class="text-2xl font-semibold font-inter <?= $text_secondary; ?> m-0 mt-0.5"><?= esc_html($duration_label); ?> listings</p>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-2.5">
                ...
                <div class="flex justify-between items-center text-[13px]">
                    <span class="<?= $text_label; ?> <?= $is_dark_card ? 'text-white/60' : 'text-gray-800'; ?> font-inter">Duration</span>
                    <span class="font-medium font-inter <?= $text_primary; ?>"><?= esc_html($duration_label); ?></span>
                </div>

                <div class="flex justify-between items-center text-[13px]">
                    <span class="<?= $text_label; ?> <?= $is_dark_card ? 'text-white/60' : 'text-gray-800'; ?> font-inter">Listings</span>
                    <span class="font-medium font-inter <?= $text_primary; ?>"><?= esc_html($props_label); ?></span>
                </div>

                <div class="flex justify-between items-center text-[13px]">
                    <span class="<?= $text_label; ?> <?= $is_dark_card ? 'text-white/60' : 'text-gray-800'; ?> font-inter">Featured listing</span>
                ...
                <div class="flex justify-between items-center text-[13px]">
                    <span class="<?= $text_label; ?> <?= $is_dark_card ? 'text-white/60' : 'text-gray-800'; ?> font-inter">Advanced analytics</span>
            ...
            <div class="mt-auto pt-1">
                <?php if (is_user_logged_in()): ?>
                    ...
                    <?php if ($is_current): ?>
                        <button disabled class="w-full py-2.5 rounded-full text-sm font-semibold bg-gray-100 text-gray-400 cursor-not-allowed">
                            Current Plan
                        </button>
